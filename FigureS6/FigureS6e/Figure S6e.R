workingDir <- "E:\\DRG_aging\\paper_Wang&Chen2025\\FigureS3\\Figure S3e/"
setwd(workingDir)
library(Seurat)
library(dplyr)
library(stringr)
...
load( "./DRG_VEC_aging_24.Rdata")

DRG_VEC_aging$age <- factor(DRG_VEC_aging$age, level = c("3 MO","24 MO"))
pdf(file = "UMAP_DRG_VEC_aging_age.pdf",width = 7,height = 5)
DimPlot(object = DRG_VEC_aging, reduction = "umap", label = TRUE, 
        split.by = "age", repel = TRUE) + coord_fixed()
dev.off()

library(sccomp)
library(tidyr)
DefaultAssay(DRG_VEC_aging) <- "RNA"
sccomp_result = 
  DRG_VEC_aging |>
  sccomp_estimate( 
...
dev.off()
write.csv(sccomp_result,file = "sccomp_result_VEC.csv")

library(RColorBrewer)
sample_anno_filter <- DRG_VEC_aging@meta.data
...
p
ggsave(filename = "barplot_subtype_age_VEC.pdf",plot = p,width = 7,height = 3)
