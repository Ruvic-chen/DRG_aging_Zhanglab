workingDir <- "E:\\DRG_aging\\paper_Wang&Chen2025\\FigureS3\\Figure S3c/"
setwd(workingDir)
library(Seurat)
library(dplyr)
library(stringr)
...
load( "./DRG_Schwann_M_aging_24.Rdata")

DRG_Schwann_M_aging$age <- factor(DRG_Schwann_M_aging$age, level = c("3 MO","24 MO"))
pdf(file = "UMAP_DRG_Schwann_M_aging_age.pdf",width = 7,height = 5)
DimPlot(object = DRG_Schwann_M_aging, reduction = "umap", label = TRUE,group.by = "seurat_clusters", 
        split.by = "age", repel = TRUE) + coord_fixed()
dev.off()

library(sccomp)
library(tidyr)
DefaultAssay(DRG_Schwann_M_aging) <- "RNA"
sccomp_result = 
  DRG_Schwann_M_aging |>
  sccomp_estimate( 
...
dev.off()
write.csv(sccomp_result,file = "sccomp_result_Sch_M.csv")

library(RColorBrewer)
sample_anno_filter <- DRG_Schwann_M_aging@meta.data
...
p
ggsave(filename = "barplot_subtype_age_Sch_M.pdf",plot = p,width = 7,height = 3)
