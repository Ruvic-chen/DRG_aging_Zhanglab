...
cds_subset <- readRDS(file = "cds_ccl8.rds")
Track_genes <- read.csv(file = "Trajectory_genes_ccl8.csv")
...
res_ccl8 <- FindMarkers(DRG_Macrophage_aging,features = merged$gene_short_name,ident.1 = "AAM_Ccl8",ident.2 = "Mac_Cd163",test.use = "MAST")
res_ccl8_1 <- subset(res_ccl8, pct.1 > 0.2)
res_ccl8_up <- rownames(res_ccl8_1[which(res_ccl8_1$avg_log2FC >= 0),])
merged$regulation <- "NA"
merged$regulation[merged$gene_short_name %in% res_ccl8_up] <- "up"

res_ccl8_1 <- subset(res_ccl8, pct.2 > 0.2)
res_ccl8_down <- rownames(res_ccl8_1[which(res_ccl8_1$avg_log2FC <= 0),])
merged$regulation[merged$gene_short_name %in% res_ccl8_down] <- "down"


# 8. 保存结果
write.csv(merged, file = "Trajectory_genes_subsampling_freq_ccl8.csv", row.names = FALSE)
