workingDir <- "E:\\DRG_aging\\paper_Wang&Chen2025\\FigureS3\\Figure S3a"
setwd(workingDir)
library(Seurat)
library(dplyr)
library(stringr)
...
load( "./DRG_Satellite_aging_24.Rdata")

DRG_Satellite_aging$age <- factor(DRG_Satellite_aging$age, level = c("3 MO","24 MO"))
pdf(file = "UMAP_DRG_Satellite_aging_age.pdf",width = 7,height = 5)
DimPlot(object = DRG_Satellite_aging, reduction = "umap", label = TRUE, 
        split.by = "age", repel = TRUE) + coord_fixed()
dev.off()

library(sccomp)
library(tidyr)
DefaultAssay(DRG_Satellite_aging) <- "RNA"
sccomp_result = 
  DRG_Satellite_aging |>
  sccomp_estimate( 
...
write.csv(sccomp_result,file = "sccomp_result_sate.csv")

library(RColorBrewer)
sample_anno_filter <- DRG_Satellite_aging@meta.data
...
p
ggsave(filename = "barplot_subtype_age_satellite.pdf",plot = p,width = 5,height = 3)
