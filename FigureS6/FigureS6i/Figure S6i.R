workingDir <- "E:\\DRG_aging\\paper_Wang&Chen2025\\FigureS3\\Figure S3i/"
setwd(workingDir)
library(Seurat)
library(dplyr)
library(stringr)
...
load( "./DRG_T_cell_aging_24.Rdata")

DRG_T_cell_aging$age <- factor(DRG_T_cell_aging$age, level = c("3 MO","24 MO"))
pdf(file = "UMAP_DRG_T_cell_aging_age.pdf",width = 7,height = 5)
DimPlot(object = DRG_T_cell_aging, reduction = "umap", label = TRUE, 
        split.by = "age", repel = TRUE) + coord_fixed()
dev.off()

library(sccomp)
library(tidyr)
DefaultAssay(DRG_T_cell_aging) <- "RNA"
sccomp_result = 
  DRG_T_cell_aging |>
  sccomp_estimate( 
...
dev.off()
write.csv(sccomp_result,file = "sccomp_result_Tcell.csv")

library(RColorBrewer)
sample_anno_filter <- DRG_T_cell_aging@meta.data
...
p
ggsave(filename = "barplot_subtype_age_Tcell.pdf",plot = p,width = 5,height = 3)
